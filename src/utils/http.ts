...
export function getUserAgent(): string {
  ...
  return `rabbit-code/${MACRO.VERSION} (${process.env.USER_TYPE ?? 'user'}, ${process.env.CLAUDE_CODE_ENTRYPOINT ?? 'cli'}${agentSdkVersion}${clientApp}${workloadSuffix})`
}
...
export function getMCPUserAgent(): string {
  ...
  return `rabbit-code/${MACRO.VERSION}${suffix}`
}

export function getWebFetchUserAgent(): string {
  return `rabbit-code/${MACRO.VERSION} (${getClaudeCodeUserAgent()})`
}
